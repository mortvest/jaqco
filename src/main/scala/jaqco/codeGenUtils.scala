...
object CodeGenUtils {
  ...
  def condTrans(expr: Expr, meta: Map[String, RelationMetaData], postFix: String): String = {
    def comparison(left: Expr, right: Expr, simpleOp: String, stringOp: String ) = {
      typeLookup(left, meta) match {
        ...
        case NoType() => throw new Error("Outside variable has no type")
      }
    }
    def trans(expr: Expr): String = {
      expr match {
        ...
        case Const(x, valType)    => valType match {
          case SimpleType(_) => x
          case StringType(_) => "\"" + x + "\""
          case NoType() => throw new Error(s"Shared Variable $x has not received a type!")
        }
  ...
  def genAssign(ty: DataType, name: String, value: String, new_struct: String) = {
    ...
    ty match {
      ...
      case NoType() => throw new Error(s"Shared Variable has not received a type!")
    }
  }
  ...
  def taggify(code: String, query: String) = {
    val tagU = "//vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv GENERATED BY JAQCO vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv"
    val tagD = "//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ GENERATED BY JAQCO ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
    query match {
      case "" => s"${tagU}\n${code}\n${tagD}"
      case query =>
        val ogQuery = s"""//${query}"""

object CodeGenFunctions {
    ...
    expr match {
      case DerefExp(alias, name) =>
        val getType = (meta: RelationMetaData) => meta.attributes get (alias + "_" + name) match {
          case None => println(meta);throw new Error(s"""Can not determine the type of variable \"${name}\" """)
  ...
  def structElem (tup: (String, DataType)) = {
    tup._2 match {
      case SimpleType(typ) => s"$typ ${tup._1};"
      case StringType(len) => s"char ${tup._1}[$len];"
    }
  }

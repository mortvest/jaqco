...
index->get(utility::encode_safe(${value}), ${meta.relName}_value_string)
...
"""
  ...
  def typeLookup(valName: String, meta: Map[String, TableMetaData]) = {
    // TODO: Should be done properly when operations on multiple relations would be added
    val lst = meta.toList
    println(lst)
    lst match {
      case (key,TableMetaData(_, _, tables ))::Nil => tables get valName
  ...
  def onePassProj(meta: Map[String, TableMetaData], exprList: List[Expr], reg: Int) = {
    def conv(expr: Expr) = {
      println(s"Matching on $expr")
      expr match {
        case Attribute(attName) => typeLookup(attName, meta) + " " + attName
    ...
    exprList.foldLeft("") ( (acc, ex) => acc + conv(ex) + "\n" ) +
    "}MACRO_PACKED;\n"
    val code = s"""

...
object Physical {
  ...
  // def parseJoin(tree: Join): List[(String, String)] = {
  ...
  //       (leftRel.getName.toString, left.getAlias.toString) ::
  //       (rightRel.getName.toString, right.getAlias.toString) :: Nil
  //     case (left: AliasedRelation, right: Join) =>
  //       val leftRel = getTable(left)
  //       (leftRel.getName.toString, left.getAlias.toString) :: parseJoin(right)
  //     case (left: Table, _) =>
  //       throw new Error(s"Alias on table ${left.getName} is required")
  //     case (_, right: Table) =>
  //       throw new Error(s"Alias on table ${right.getName} is required")
  ...
  def apply(query: Query, meta: Map[String, TableMetaData]) = {
    ...
    toScala(body.getFrom).get match {
      case x: Table =>
        ...
        fun(cond, aliasedProj, name, name)
      case x: AliasedRelation =>
        ...
        fun(cond, aliasedProj, getTable(x).getName.toString, x.getAlias.toString)
      case x: Join =>

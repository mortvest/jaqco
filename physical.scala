...
case class OnePassProj(meta: Map[String, TableMetaData], exprList: List[Expr], from: Physical) extends Physical
...
object Physical{
  def apply(relAlg: RelAlg, meta: Map[String, TableMetaData]): Physical = {
    relAlg match {
      ...
      case Projection(attList, from) => OnePassProj(meta, attList, apply(from, meta))
      case _ => throw new Error("Not implemented yet")
    }
  }
  def getMeta(meta: Map[String, TableMetaData], name: String) = {
    meta get name match {
      case None => print(name); print(meta); throw new Error("Relation with that name does not exist")
      case Some(x) => x
    }
  }
  def translateCond(cond: Expr, meta: TableMetaData): Physical = {
    cond match {
      ...
      case Equals(Attribute(attName1), Attribute(attName2))
          if (meta.indexName == attName1) && !(meta.tables.contains(attName2)) =>
        IndexScan(meta, Attribute(attName2))
      case Equals(Attribute(attName1), Attribute(attName2))
          if (meta.indexName == attName2) && !(meta.tables.contains(attName1)) =>
        IndexScan(meta, Attribute(attName1))
